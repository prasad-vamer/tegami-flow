...
export function extractTemplateVariables(template: TemplateWithContent | null): string[] {
  if (!template) return []
...
export function getCandidateValueForVariable(candidate: Candidate | null, variable: string): string {
  if (!candidate) return ""
